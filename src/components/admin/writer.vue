      ...
      methods:{
        ...
        caozuo:function () {
          if(this.title == "添加"){
            ...
            this.$http.post(`http://localhost:8088/aiyue/writer/updateWriter?wid=${this.writer.wid}&wname=${this.writer.wname}&wphoto=${this.writer.wphoto}&ana=${this.writer.ana}`)
              .then((resp) => {
                if(resp.data == 1) {
                  this.query();
                }else if(resp.data == 2){
